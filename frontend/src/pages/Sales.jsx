      ...
      <div className="rounded-lg border border-slate-200 bg-white shadow-sm overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {!loading && sales.map((s) => (
              <tr key={s._id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                <td className="px-4 py-3 font-mono text-xs text-indigo-600 font-medium">{s.invoiceNumber}</td>
                <td className="px-4 py-3 text-slate-500 text-xs">{date(s.saleDate)}</td>
                <td className="px-4 py-3">
                  <p className="font-medium text-slate-900">{s.customer.name}</p>
                  <p className="text-xs text-slate-400">{s.customer.phone}</p>
                </td>
                <td className="px-4 py-3 text-slate-600">
                  <p>{s.items.length} laptop{s.items.length !== 1 ? 's' : ''}</p>
                  {s.items[0] && <p className="text-xs text-slate-400">{s.items[0].brand} {s.items[0].model}</p>}
                </td>
                <td className="px-4 py-3 font-semibold text-slate-900">{currency(s.grandTotal)}</td>
                {user?.role === 'admin' && (
                  <td className="px-4 py-3 text-emerald-600 font-medium text-xs">{currency(s.profit)}</td>
                )}
                <td className="px-4 py-3 text-xs text-slate-500">{s.paymentMethod}</td>
                <td className="px-4 py-3 text-xs text-slate-500">{s.salesperson?.name || s.salespersonName}</td>
                <td className="px-4 py-3">
                  <span className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium ${s.status === 'cancelled' ? 'bg-red-100 text-red-600' : 'bg-emerald-100 text-emerald-700'}`}>
                    {s.status}
                  </span>
                </td>
                <td className="px-4 py-3">
                  <div className="flex gap-1">
                    <button onClick={() => openInvoice(s._id)} title="View Invoice" className="rounded p-1.5 text-slate-400 hover:bg-slate-100 hover:text-indigo-600">
                      <FileText className="h-4 w-4" />
                    </button>
                    {user?.role === 'admin' && s.status !== 'cancelled' && (
                      <button onClick={() => { setCancelTarget(s); setCancelReason(''); }} title="Cancel Sale" className="rounded p-1.5 text-slate-400 hover:bg-red-50 hover:text-red-500">
